...
2. Copy `.env.example` to `.env` and add your keys:
   ...
   - `PAYPAL_CLIENT_ID` (public client id; `PAYPAL_CLIENT_SECRET` and `PAYPAL_MODE` are Supabase Edge Function secrets)
3. Run `flutter pub get`.
4. Apply Supabase migrations: `supabase db push`.
5. Deploy Edge Functions (`create-paypal-payment`, `complete-paypal-order`, `delete-account`, plus existing push/email). Set `PAYPAL_CLIENT_ID`, `PAYPAL_CLIENT_SECRET`, and `PAYPAL_MODE` (`sandbox` or `live`) in the Supabase dashboard. Stripe postage functions can stay deployed but are unused by the app UI.
...
- Release builds fail closed without Supabase / RevenueCat / PayPal (no fake donation, order, or login success).
...
**Payments:** Donations use **RevenueCat** (store IAP). Postage uses **PayPal**. Keys live in Flutter `.env` / Codemagic and Supabase secrets — not in the admin. The admin stores offering/package IDs in `app_settings.revenuecat` only.
...
- `lib/core/services/` — Supabase, RevenueCat, PayPal postage, Firebase, Quran API
